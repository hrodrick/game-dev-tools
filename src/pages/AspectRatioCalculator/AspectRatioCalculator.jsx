...
const COMMON_RATIOS = [
  {
    label: "16:9 (HD, FHD, 4K, etc)", value: 16 / 9, w: 16, h: 9,
    resolutions: [
  ...
  {
    label: "4:3 (Classic)", value: 4 / 3, w: 4, h: 3,
    resolutions: [
  ...
  {
    label: "21:9 (Ultrawide)", value: 21 / 9, w: 21, h: 9,
    resolutions: [
...
export default function AspectRatioCalculator() {
  ...
  return (
    <div style={{ maxWidth: 800, margin: "40px auto", padding: 24, background: "#fafbfc", borderRadius: 10, boxShadow: "0 2px 12px #0001" }}>
      <h2>Aspect Ratio Calculator</h2>
      <div style={{ marginBottom: 20 }}>
        <label>
          Width:
          <input type="number" value={width} min={1} onChange={e => setWidth(Number(e.target.value))} style={{ width: 80, marginLeft: 10 }} />
        </label>
        <label style={{ marginLeft: 24 }}>
          Height:
          <input type="number" value={height} min={1} onChange={e => setHeight(Number(e.target.value))} style={{ width: 80, marginLeft: 10 }} />
        </label>
      </div>
              ...
              <li key={r.label}>
                <button
                  style={{
                    ...
                    padding: "4px 10px",
                    borderRadius: 5,
                    border: r.label === selectedRatio.label ? "2px solid #4078c0" : "1px solid #bbb",
                    background: r.label === selectedRatio.label ? "#eaf3ff" : "#fff",
                    color: r.label === selectedRatio.label ? "#4078c0" : "inherit",
                    cursor: "pointer",
        ...
        <div style={{ flex: 1 }}>
          ...
          <ul style={{ margin: "8px 0 0 0", padding: 0, listStyle: "none" }}>
            {selectedResolutions.map(r => (
              <li key={r.label}>
                <button
                  style={{ margin: "4px 0", padding: "4px 10px", borderRadius: 5, border: "1px solid #bbb", background: "#fff", cursor: "pointer" }}
                  onClick={() => {
                    setWidth(r.w);
                    setHeight(r.h);
                  }}
